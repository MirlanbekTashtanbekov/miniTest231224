...
drop type familystatus cascade ;
...
alter table agencies add constraint set check('+996');

alter table customers rename family_status to marital_status;
alter table customers add constraint set unique (email);

alter table owners add constraint a unique (email);
...
select house_type,price from houses where price>1500 and price<2000;
-- 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
select * from addresses join houses on addresses.id = houses.address_id where addresses.id in (5,6,7,8,9,10);
...
select * from customers where date_of_birth>'1999-01-01' limit 15 offset 10;
-- 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
select house_type, rating, owner_id from houses join owners on houses.owner_id = owners.id order by rating  desc;
-- 7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
select count(*), sum(price) from houses where house_type='Apartment' group by house_type;
...
select * from agencies where name = 'My House';
...
-- 17.Улуту Kyrgyzstan уй-булолу customerлерди чыгарыныз.
    select * from customers where nationality = 'Kygyzstan';
